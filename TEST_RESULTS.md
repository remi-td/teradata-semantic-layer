...
| --- | --- | --- | --- | --- | --- | --- |
| B01 | Baseline | Revenue by customer market segment (2-hop) | PASS | NOT_SUPPORTED | MISMATCH | 01_baseline.yaml |
| B02 | Baseline | Single-table AVG (avg_qty by return flag) | PASS | PASS | MATCH | 01_baseline.yaml |
| B03 | Baseline | 5-hop traversal with all intermediates named (revenue by region) | PASS | NOT_SUPPORTED | MISMATCH | 01_baseline.yaml |
| F01 | Filters | Date filter with DATE literal | PASS | NOT_SUPPORTED | MISMATCH | 02_filters.yaml |
| F02 | Filters | LIKE filter (priority starts with 1-) | PASS | NOT_SUPPORTED | MISMATCH | 02_filters.yaml |
| F03 | Filters | IN list (post-F1 — caller supplies parens, compiler does not wrap) | PASS | NOT_SUPPORTED | MISMATCH | 02_filters.yaml |
| F04 | Filters | Filter on region.r_name with region not in dims (F2 auto-include) | PASS | NOT_SUPPORTED | MISMATCH | 02_filters.yaml |
| F05 | Filters | Unquoted string value (caller error — must still fail cleanly) | RUNTIME_ERROR | RUNTIME_ERROR | MATCH | 02_filters.yaml |
| R01 | Ratio | promo_share by region (5-hop + part) | PASS | NOT_SUPPORTED | MISMATCH | 03_ratio_and_having.yaml |
| R02 | HAVING | HAVING promo_share > 0.1 | PASS | NOT_SUPPORTED | MISMATCH | 03_ratio_and_having.yaml |
| P01 | BFS | Auto-intermediates — revenue by region with NO intermediates named | PASS | PASS | MATCH | 04_path_resolution.yaml |
| P02 | Ambiguous path | Ambiguous path — revenue by supplier direct, NOT via partsupp | PASS | NOT_SUPPORTED | MISMATCH | 04_path_resolution.yaml |
...
| C01 | Chasm trap | revenue + total_availqty by part — compiler flags CHASM_WARNING | COMPILE_REJECTED | NOT_SUPPORTED | MISMATCH | 06_chasm_trap.yaml |
| FO01 | Fan-out | count_orders grouped at lineitem grain (DISTINCT survives fan-out) | PASS | NOT_SUPPORTED | MISMATCH | 07_fanout.yaml |
| TG01 | Time grain | No grain (day-level, default) | PASS | NOT_SUPPORTED | MISMATCH | 08_time_grain.yaml |
| TG02 | Time grain | Monthly rollup (:MONTH) | PASS | NOT_SUPPORTED | MISMATCH | 08_time_grain.yaml |
...
| E02 | Edge shapes | Dimension-only (distinct values of one dim, F3) | PASS | NOT_SUPPORTED | MISMATCH | 09_edge_shapes.yaml |
...
| id | expected | actual | title |
| --- | --- | --- | --- |
| B01 | PASS | NOT_SUPPORTED | Revenue by customer market segment (2-hop) |
| B03 | PASS | NOT_SUPPORTED | 5-hop traversal with all intermediates named (revenue by region) |
| F01 | PASS | NOT_SUPPORTED | Date filter with DATE literal |
| F02 | PASS | NOT_SUPPORTED | LIKE filter (priority starts with 1-) |
| F03 | PASS | NOT_SUPPORTED | IN list (post-F1 — caller supplies parens, compiler does not wrap) |
| F04 | PASS | NOT_SUPPORTED | Filter on region.r_name with region not in dims (F2 auto-include) |
| R01 | PASS | NOT_SUPPORTED | promo_share by region (5-hop + part) |
| R02 | PASS | NOT_SUPPORTED | HAVING promo_share > 0.1 |
| P02 | PASS | NOT_SUPPORTED | Ambiguous path — revenue by supplier direct, NOT via partsupp |
| C01 | COMPILE_REJECTED | NOT_SUPPORTED | revenue + total_availqty by part — compiler flags CHASM_WARNING |
| FO01 | PASS | NOT_SUPPORTED | count_orders grouped at lineitem grain (DISTINCT survives fan-out) |
| TG01 | PASS | NOT_SUPPORTED | No grain (day-level, default) |
| TG02 | PASS | NOT_SUPPORTED | Monthly rollup (:MONTH) |
| E02 | PASS | NOT_SUPPORTED | Dimension-only (distinct values of one dim, F3) |
| SL01 | PASS | NOT_SUPPORTED | Top-5 customers by revenue (ORDER BY alias + TOP) |
| MD01 | PASS | NOT_SUPPORTED | promo_revenue alone (needs part via METRIC_FIELD_REF) |
