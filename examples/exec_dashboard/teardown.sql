-- Remove all catalog rows for model 'exec_dashboard'. Deletes are leaf-to-root to
-- respect integrity. Run via: semantic-catalog uninstall-example exec_dashboard
...
DELETE FROM demo_user.METRIC
 WHERE model_id IN (SELECT model_id FROM demo_user.SEMANTIC_MODEL WHERE model_name = 'exec_dashboard');
...
DELETE FROM demo_user.AI_CONTEXT
 WHERE (entity_type='DATASET' AND entity_id IN (SELECT d.dataset_id FROM demo_user.DATASET d
    ...
    OR (entity_type='MODEL' AND entity_id IN (SELECT model_id FROM demo_user.SEMANTIC_MODEL
           WHERE model_name = 'exec_dashboard'));
...
DELETE FROM demo_user.SEMANTIC_MODEL WHERE model_name = 'exec_dashboard';
