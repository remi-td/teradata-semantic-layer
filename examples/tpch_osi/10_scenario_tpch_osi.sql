...
VALUES ('customer', 'Customer master. One row per customer.', 'One row per customer.', 'tpch', 'customer');

INSERT INTO demo_user.DATASET (dataset_name, description, granularity_desc, DataBaseName, TableName)
VALUES ('orders', 'Order header. One row per customer order.', 'One row per order.', 'tpch', 'orders');

INSERT INTO demo_user.DATASET (dataset_name, description, granularity_desc, DataBaseName, TableName)
VALUES ('lineitem', 'Order line items. Fact grain: one row per (order, line number).', 'One row per (l_orderkey, l_linenumber).', 'tpch', 'lineitem');

INSERT INTO demo_user.DATASET (dataset_name, description, granularity_desc, DataBaseName, TableName)
VALUES ('part', 'Part catalog. One row per part.', 'One row per part.', 'tpch', 'part');

INSERT INTO demo_user.DATASET (dataset_name, description, granularity_desc, DataBaseName, TableName)
VALUES ('nation', 'Nations. One row per nation.', 'One row per nation.', 'tpch', 'nation');

INSERT INTO demo_user.DATASET (dataset_name, description, granularity_desc, DataBaseName, TableName)
VALUES ('region', 'Regions. One row per region.', 'One row per region.', 'tpch', 'region');

-- Link all datasets to the tpch_osi model
...
WHERE m.model_name='tpch_osi'
  AND d.dataset_name IN ('customer','orders','lineitem','part','nation','region');
...
FROM demo_user.SEMANTIC_MODEL m
INNER JOIN demo_user.MODEL_DATASET md2 ON md2.model_id=m.model_id INNER JOIN demo_user.DATASET d ON d.dataset_id=md2.dataset_id AND d.dataset_name='customer'
WHERE m.model_name='tpch_osi';
