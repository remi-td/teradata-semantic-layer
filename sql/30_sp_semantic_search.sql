...
--
-- Implemented as a parameterized MACRO so it can be invoked directly from
-- any client and returns rows natively — unlike stored procedure dynamic
-- result sets which some clients cannot consume.
--
    ...
    SELECT * FROM (
        ...
        SELECT CAST('DATASET' AS VARCHAR(20))                              AS entity_type,
          ...
          LEFT JOIN demo_user.AI_CONTEXT ac
                 ON ac.entity_type = 'DATASET' AND ac.entity_id = d.dataset_id
         WHERE (:p_model_name IS NULL OR m.model_name = :p_model_name)
           AND ( LOWER(d.dataset_name)                                         LIKE '%' || LOWER(:p_term) || '%'
              OR LOWER(COALESCE(d.description,''))                             LIKE '%' || LOWER(:p_term) || '%'
              OR LOWER(COALESCE(ac.display_name,''))                           LIKE '%' || LOWER(:p_term) || '%'
              OR LOWER(COALESCE(ac.instructions,''))                           LIKE '%' || LOWER(:p_term) || '%'
              OR LOWER(COALESCE(CAST(ac.synonyms AS VARCHAR(1000)),''))        LIKE '%' || LOWER(:p_term) || '%' )
        ...
        SELECT CAST('FIELD' AS VARCHAR(20)),
         ...
         WHERE (:p_model_name IS NULL OR m.model_name = :p_model_name)
           AND ( LOWER(f.field_name)                                           LIKE '%' || LOWER(:p_term) || '%'
              OR LOWER(COALESCE(f.label,''))                                   LIKE '%' || LOWER(:p_term) || '%'
              OR LOWER(COALESCE(f.description,''))                             LIKE '%' || LOWER(:p_term) || '%'
              OR LOWER(COALESCE(ac.display_name,''))                           LIKE '%' || LOWER(:p_term) || '%'
              OR LOWER(COALESCE(CAST(ac.synonyms AS VARCHAR(1000)),''))        LIKE '%' || LOWER(:p_term) || '%' )
        ...
        SELECT CAST('METRIC' AS VARCHAR(20)),
         ...
         WHERE (:p_model_name IS NULL OR m.model_name = :p_model_name)
           AND ( LOWER(mt.metric_name)                                         LIKE '%' || LOWER(:p_term) || '%'
              OR LOWER(COALESCE(mt.description,''))                            LIKE '%' || LOWER(:p_term) || '%'
              OR LOWER(COALESCE(ac.display_name,''))                           LIKE '%' || LOWER(:p_term) || '%'
              OR LOWER(COALESCE(ac.instructions,''))                           LIKE '%' || LOWER(:p_term) || '%'
              OR LOWER(COALESCE(CAST(ac.synonyms AS VARCHAR(1000)),''))        LIKE '%' || LOWER(:p_term) || '%' )
        ...
        SELECT CAST('VIEW' AS VARCHAR(20)),
          ...
          LEFT JOIN demo_user.AI_CONTEXT ac
                 ON ac.entity_type = 'VIEW' AND ac.entity_id = v.view_id
         WHERE (:p_model_name IS NULL OR m.model_name = :p_model_name)
           AND ( LOWER(v.view_name)                                           LIKE '%' || LOWER(:p_term) || '%'
              OR LOWER(COALESCE(v.description,''))                            LIKE '%' || LOWER(:p_term) || '%'
              OR LOWER(COALESCE(ac.display_name,''))                          LIKE '%' || LOWER(:p_term) || '%'
              OR LOWER(COALESCE(CAST(ac.synonyms AS VARCHAR(1000)),''))       LIKE '%' || LOWER(:p_term) || '%' )
