...
class CatalogDAO(Protocol):
    ...
    def list_model_names(self) -> List[str]:
        """Return every model name in the catalog (used for UNKNOWN_MODEL
        suggestions). Implementations that don't support enumeration may
    ...
    def list_field_tokens(self, model_id: int) -> List[str]:
        """Return every dimension-style token in the model: bare field
        names plus their ``dataset.field`` qualified form. Used to build
        suggestions on UNKNOWN_ENTITY for dimensions / WHERE filters."""
        ...
    ...
    def list_metric_names(self, model_id: int) -> List[str]:
        """Return every metric name in the model (UNKNOWN_ENTITY suggestions)."""
        ...
